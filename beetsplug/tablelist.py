...
def make_table( results: Results, config: Configuration, use_album_format: bool = False ) -> Table:
    table_style = make_style( config )
    table = Table( **table_style )

    fields = config['album_columns'].get( str ) if use_album_format else config['item_columns'].get( str )
